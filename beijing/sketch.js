...
function preload() {
  data = loadTable("webapp_beijing.csv","csv","header");
  bgImage = loadImage("beijing.png");
  flagImage = loadImage("flag.png");
}
function setup() {
  
  createCanvas(1175, 821);
  ...
  for(let i = 0; i < numRows; i++){
    let mapLng = map(lng[i], minLng,maxLng,82,1072);
    let mapLat = map(lat[i],minLat,maxLat,769,40);
    names[i] = name[i];
    means[i] = mean[i];
    addrs[i] = addr[i];
